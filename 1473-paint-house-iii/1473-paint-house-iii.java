        ...
        for (int i = 1; i < m; i++){
            ...
            for (int j = 1; j <= Math.min(T, i); j++){ // num of neighborhood
                for (int k = 1; k <= n; k++){ // current color
                    if (houses[i]>0){ // can't paint this, so compare with previous house.
                        int v = k == houses[i]?0:1;
                        next[j+v][houses[i]]=Math.min(next[j+v][houses[i]], dp[j][k]);
                        continue;
                    }
                    if (houses[i-1]!=0){ // previous house is already determined, so just compare it here
                        int v = houses[i-1]==k?0:1;
                        next[j+v][k]=Math.min(next[j+v][k], dp[j][houses[i-1]]+cost[i][k-1]);
                        continue;
                    }
                    for (int p = 1; p <= n; p++){ // otherwise, loop through all previous colors.
                        int v = p == k? 0 : 1;
